...
async function handleScheduleCommand(argv: string[], json: boolean): Promise<CliResult> {
  ...
  if (command === "open" && targetId) {
    ...
    // launchCopilot) rooted at the schedule state dir, so the SessionStart
    // [SCHEDULE RESULTS] banner surfaces this job's latest run — "ready to engage".
