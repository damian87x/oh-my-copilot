...
export async function runCli(argv = process.argv.slice(2)): Promise<CliResult> {
  ...
  if (group === "update") {
    ...
    // Refresh user-home install (hooks + skills + agents). Never scaffold the
    ...
    try {
      ...
      const bundlePart =
        bundleWrites > 0
          ? `User skills/agents refreshed (${bundleWrites} file(s)).`
          : "User skills/agents: up to date (or no bundle).";
      homeMsg = `\n${hookPart}\n${bundlePart}`;
    } catch {
      homeMsg = "\nUser install refresh skipped (run: omp setup).";
    }
  ...
  if (group === "setup") {
    ...
    const baseOpts = {
      ...
      // Default user home; only --scope project writes into the repo.
      scope: (flagValue(argv, "--scope") === "project" ? "project" : "user") as "user" | "project",
    };
  ...
  if (group === "config") {
    ...
    if (command === "set") {
      ...
      if (numericKey) {
        ...
        // Directive budget changes must regenerate the static fallback block,
        // or headless sessions keep the old cap's directive set indefinitely.
        if (numericKey.key === "memoryDirectiveCap" || numericKey.key === "memoryDirectiveCharCap") {
          const { syncInstructionsMemory } = await import("./instructions-memory.js");
  ...
  if (group === "project-memory") {
    ...
    if (command === "add-directive") {
      ...
      const homeDir = process.env.OMP_HOME_OVERRIDE || undefined;
      const { memoryDirectiveCap: cap, memoryDirectiveCharCap: charCap } = readMemoryConfig(cwd, { homeDir });
      const warns: string[] = [];
      if (count > cap) {
        warns.push(`${count} directives stored but only the first ${cap} inject at session start; consolidate or raise memory-directive-cap`);
      }
      if (directive.trim().length > charCap) {
        warns.push(`this directive is ${directive.trim().length} chars, over the ${charCap}-char budget (memory-directive-char-cap) — it will never inject`);
      }
      const warn = warns.length ? ` — WARNING: ${warns.join("; ")}` : "";
      return json
        ? { ok: true, output: { ok: true, count, injectionCap: cap, injectionCharCap: charCap, overCap: count > cap } }
        : { ok: true, message: `directive added (${count} total)${warn}` };
    }
    if (command === "pending") {
      const q = await import("./memory-review/quarantine.js");
      q.migrateLegacyQuarantine(cwd); // surface pre-migration proposals, not just .omp ones
    ...
    if (command === "promote-directive" || command === "dismiss-directive") {
      ...
      if (command === "promote-directive") {
        // Skip proposals already active: a retry after a crash between add and
        // dequeue (or a manual add-directive of the same rule) must not
        // duplicate the rule and burn injection-cap slots.
        const normalize = (s: string) => s.trim().toLowerCase().replace(/\s+/g, " ");
        const active = new Set(pm.readDirectives(cwd).map(normalize));
        const fresh = selected.filter((t) => !active.has(normalize(t)));
        const dupes = selected.length - fresh.length;
        for (const text of fresh) pm.addDirective(cwd, text);
        syncInstructionsMemory(cwd); // refresh the managed block Copilot reads
        // Dequeue by TEXT identity, not index: a concurrent dismiss/promote of
        // an earlier item shifts positions, and index-based removal would then
        // hit the wrong line. Crash order stays add-first: a failure here
        // leaves a visible duplicate in the queue instead of a silent loss.
        const removed = q.removePendingDirectives(cwd, selected);
        const missed = selected.length - removed.length;
        // Same budget honesty as add-directive: promotion can push past the cap too.
        const { readMemoryConfig } = await import("./memory-review/config.js");
        const homeDir = process.env.OMP_HOME_OVERRIDE || undefined;
        const cap = readMemoryConfig(cwd, { homeDir }).memoryDirectiveCap;
        const total = pm.readDirectives(cwd).length;
        const warn = total > cap
          ? ` — WARNING: ${total} directives stored but only the first ${cap} inject at session start; consolidate or raise memory-directive-cap`
          : "";
        const notes: string[] = [];
        if (dupes > 0) notes.push(`${dupes} already active, skipped`);
        if (missed > 0) notes.push(`${missed} queue line${missed === 1 ? " was" : "s were"} already gone (concurrent edit?)`);
        const suffix = notes.length ? `\n(${notes.join("; ")})` : "";
        return json
          ? { ok: true, output: { promoted: fresh, skippedDuplicates: dupes, dequeued: removed, overCap: total > cap } }
          : { ok: true, message: `promoted ${fresh.length} directive${fresh.length === 1 ? "" : "s"}:\n${fresh.map((d) => `- ${d}`).join("\n")}${suffix}${warn}` };
      }
      const removed = q.removePendingDirectives(cwd, selected);
      return json
    ...
    if (command === "search") {
      const query = value && !value.startsWith("-") ? value : undefined;
      if (!query) {
        return { ok: false, exitCode: 1, message: 'usage: omp project-memory search "<query>" [--topic <id>] [--limit <n>]' };
      }
      const { searchMemory } = await import("./memory-recall.js");
      ...
      const results = searchMemory(cwd, { query, topic: flagValue(argv, "--topic"), limit });
      return json
        ? { ok: true, output: { results } }
        : {
            ok: true,
            message: results.length
              ? results.map((r) => `[${r.source}] ${r.title} (${r.id})\n  ${r.preview}`).join("\n")
              : "(no matches)",
          };
    }
  ...
  if (group === "skill" && command === "install" && value) {
    ...
    try {
      const result = installSkill({
        ...
        source: value,
        // Default user home; --scope project is the only path into the repo.
        scope: flagValue(argv, "--scope") === "project" ? "project" : "user",
        dryRun: hasFlag(argv, "--dry-run"),
      });
