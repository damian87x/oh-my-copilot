...
export function buildReviewPrompt(messages: TranscriptMessage[]): string {
  ...
  return [
    ...
    "",
    "Extract:",
    "- directives: STANDING behavioral preferences, captured ONLY when the user CORRECTED your behavior (e.g. 'stop doing X', 'too verbose', 'you always Y') or expressed a preference that recurred across the session. Do NOT promote a one-off instruction scoped to the current task (e.g. 'format THIS answer as bullets') into a directive. Declarative form ('User prefers concise replies'). Empty unless a correction or standing preference is clearly evidenced.",
    "- notes: durable FACTS about the project or environment worth recalling later (architecture, conventions, gotchas, where things live). Do NOT save session-outcome facts: what files changed this session, commit SHAs, PR/issue numbers, 'tests passed', task-completion status, file counts, or anything that will be stale in 7 days. If it will be stale in 7 days, it is NOT a note.",
    "- skill_drafts: reusable PROCEDURES or workflows that emerged (slug in kebab-case, reason, body as markdown). This is where 'how we did X' belongs — not notes.",
    "",
