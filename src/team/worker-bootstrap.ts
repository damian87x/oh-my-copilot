...
export function buildInboxMarkdown(opts: InboxOptions): string {
  ...
  return [
    ...
    "",
    "1. Claim your task:",
    `   omc team api claim-task --input '${claimInput}' --json`,
    "   Save the claim_token from the response.",
    "",
    "2. Do the work described below.",
    "",
    "3. On completion (use claim_token from step 1):",
    `   omc team api transition-task-status --input '${completeInputTemplate}' --json`,
    "",
    "4. On failure:",
    `   omc team api transition-task-status --input '${failInputTemplate}' --json`,
    "",
    "## Task Assignment",
