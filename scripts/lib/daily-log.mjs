...
/** The project's durable objective from .omp/goal.md, or null when unset. */
export function readProjectGoal(directory) {
  try {
    const p = join(ompRoot(directory), ".omp", "goal.md");
    if (!existsSync(p)) return null;
    const text = readFileSync(p, "utf8");
    const lines = (text.charCodeAt(0) === 0xfeff ? text.slice(1) : text).split("\n");
    // Preserve the historical header on disk so this terminology cutover does
